...
impl Artifact for KeystrokeEntry {
    ...
    fn validate_plausibility(&self) -> Result<()> {
        ...
        if self.keystrokes.is_empty() {
            return Err(EngineError::ImplausibleArtifact { reason: "empty keystroke sequence".into() });
        }
        if self.avg_cpm <= 0.0 || self.avg_cpm > 1000.0 {
            return Err(EngineError::ImplausibleArtifact { reason: format!("unrealistic CPM: {}", self.avg_cpm) });
        }
        Ok(())
    }
...
const COMMON_WORDS: &[&str] = &[
    ...
    "search", "help", "click", "open", "close", "save", "delete", "send",
    "hello", "thanks", "please", "yes", "no", "okay", "sure", "great",
];

pub struct KeystrokeGenerator;
impl KeystrokeGenerator { pub fn new() -> Self { Self } }
impl Default for KeystrokeGenerator { fn default() -> Self { Self::new() } }
    ...
    fn generate(&self, _profile: &UserProfile, context: &GenerationContext, rng: &mut (impl RngCore + CryptoRng)) -> Result<Box<dyn Artifact>> {
        ...
        for w in 0..word_count {
            ...
            for ch in word.chars() {
                ...
                let flight = if keystrokes.is_empty() { 0 } else {
        ...
        let avg_cpm = if duration_min > 0.0 { total_chars as f64 / duration_min } else { 200.0 };
        let error_rate = if total_chars > 0 { errors as f64 / total_chars as f64 * 100.0 } else { 0.0 };

        let meta = ArtifactMetadata::new(DataCategory::Input, context.now, context.now, keystrokes.len() as u64 * 32)?;

        let entry = KeystrokeEntry { meta, keystrokes, avg_cpm, error_rate };
        entry.validate_plausibility()?;
        Ok(Box::new(entry))
    }

    fn category(&self) -> DataCategory { DataCategory::Input }
...
mod tests {
    ...
    fn test_dwell_times_realistic() {
        ...
        for k in &e.keystrokes {
            assert!(k.dwell_ms >= 40 && k.dwell_ms <= 200, "dwell {} out of range", k.dwell_ms);
        }
    }
    ...
    fn test_timestamps_ordered() {
        ...
        for w in e.keystrokes.windows(2) {
            assert!(w[1].press_time_ms >= w[0].press_time_ms, "keystrokes not ordered");
        }
    }
}
