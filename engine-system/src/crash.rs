...
const CRASH_SCENARIOS: &[(&str, &str, i32, &str)] = &[
    (
        ...
        "SIGSEGV",
        139,
        "mozilla::dom::ContentChild::RecvLoadURL",
    ),
    ...
    (
        ...
        "SIGABRT",
        134,
        "Py_FatalError: _Py_HashRandomization_Init",
    ),
...
impl Default for CrashGenerator {
    fn default() -> Self {
        Self::new()
    }
}
...
impl DataGenerator for CrashGenerator {
    fn generate(
        ...
        let entry = CrashEntry {
            ...
            exit_code: *code,
            timestamp,
            core_dump_path: if has_core {
                Some(format!("/var/lib/systemd/coredump/core.{process}.{pid}"))
            } else {
                None
            },
